  ...
  constructor(scope, id, props) {
    ...
    const pickFileTransfersLambda = new lambda.Function(
      ...
      `${props.resourcePrefix}pickFileTransfersLambda`,
      {
        ...
        environment: {
          auditTable: `${resourcePrefix}${dbTables.FileTransferAuditTable}`,
          REGION: this.region,
        },
      }
    );
    ...
    const getAllFileTransferTypeLambda = new lambda.Function(
      this,
      `${props.resourcePrefix}getAllFileTransferTypeLambda`,
      {
        runtime: props.runtime,
        layers: allLayersList,
        role: lambdaRole,
        functionName: `${props.resourcePrefix}getAllFileTransferTypeLambda`,
        code: lambda.Code.asset("lambdas/fileTransfers/getAllFileTransferTypes"),
        handler: "getAllFileTransferTypes.handler",
        timeout: cdk.Duration.seconds(300),
    ...
    api.root
      .resourceForPath("lambdas/fileTransfers/dashboard")
    ...
    api.root
      .resourceForPath("lambdas/fileTransfers/{fileName}")
    ...
    api.root
      .resourceForPath("lambdas/fileTransfers/search")
    ...
    api.root
      .resourceForPath("lambdas/fileTransfers/lov")
      .addMethod("GET", new apigw.LambdaIntegration(getAllFileTransferTypeLambda), {
        // authorizationType: apigw.AuthorizationType.COGNITO,
        // authorizer: {authorizerId: props.authorizer.attrAuthorizerId},
      });
    api.root
      .resourceForPath("lambdas/fileTransfers/syncBox")
      .addMethod("POST", new apigw.LambdaIntegration(syncFileTransfersLambda));
    api.root
      .resourceForPath("lambdas/fileTransfers/submitTransfers")
      .addMethod("POST", new apigw.LambdaIntegration(submitFileTransfersLambda));
    api.root
      .resourceForPath("lambdas/fileTransfers/initPick")
      .addMethod("POST", new apigw.LambdaIntegration(pickFileTransfersLambda));
    api.root
      .resourceForPath("lambdas/fileTransfers/updateTsf")
      .addMethod("POST", new apigw.LambdaIntegration(updateFileTransfersLambda));

    api.root
      .resourceForPath("lambdas/fileTransfers/getSyncedTsf")
